...
const postTextArg = process.argv[2];
const imagePath = process.argv[3];

if (!postTextArg) {
  consola.error('使い方: npx tsx index.ts "投稿テキスト" [画像ファイルパス]');
  consola.error('例: npx tsx index.ts "こんにちは！"');
  consola.error('例: npx tsx index.ts "今日の一枚" ./photo.jpg');
  process.exit(1);
}
const postText: string = postTextArg;
